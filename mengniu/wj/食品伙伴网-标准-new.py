...
class foodmateStandard(object):
    ...
    def scheduler(self):
        column_list = [
            # ("3", "食品伙伴网-标准-国内标准-国家标准", 50, 600),
            # ("4", "食品伙伴网-标准-国内标准-行业标准", 20, 130),
            # ("15", "食品伙伴网-标准-国内标准-地方标准", 30, 1000),
            # # # # # ("qiyebiaozhun", "食品伙伴网-标准-国内标准-企业标准"),
            # ("12", "食品伙伴网-标准-国内标准-团体标准", 30, 800),
            # ("9", "食品伙伴网-标准-国内标准-其他国内标准", 10, 170),
            # # # # ("2", "食品伙伴网-标准-国外标准"),
            ("8", "食品伙伴网-标准-国内标准-轻工标准", 20, 66),
        ]
        ...
        while retry < 10:
            ...
            try:
                ...
                for div in div_list:
                    ...
                    find_detail_url = mengniu_data_original_col.find_one({"_id": self.detail_url}, {"标题": 1, "B_标准状态": 1})
                    if find_detail_url:
                        if self.title.replace(" ", "") == find_detail_url["标题"].replace(" ", "") and self.F_有效性状态 == find_detail_url["B_标准状态"]:
                            # logger.info(f'已经采集过 且内容无变化： {self.publish_time}, {self.detail_url}, {self.title}')
                            continue
                        else:
                            logger.info(f'已经采集过 内容有变 需要更新: {self.publish_time}, {self.detail_url}, title: {self.title} :: {find_detail_url["标题"]}, 状态: {self.F_有效性状态} :: {find_detail_url["B_标准状态"]}')
    ...
    def get_detail(self):
        ...
        while retry < 5:
            ...
            try:
                ...
                self.title = "".join([title_text.strip() for title_text in response_html.xpath('//div[@class="title2"]/span//text()')])
                B_标准名称 = self.title
