...
class foodmateStandard(object):
    ...
    def scheduler(self):
        column_list = [
            ("6", "食品伙伴网-标准-国内标准-国家标准"),
            # ("7", "食品伙伴网-标准-国内标准-行业标准"),
            # ("12", "食品伙伴网-标准-国内标准-地方标准"),
            # ("60", "食品伙伴网-标准-国内标准-企业标准"),
            # ("24", "食品伙伴网-标准-国内标准-团体标准"),
            # ("17", "食品伙伴网-标准-国内标准-其他国内标准"),
            # ("5", "食品伙伴网-标准-国外标准"),
        ]
        ...
        while True:
            ...
            try:
                params = {
                    ...
                    'corpstandard': '2',
                    'bzstatus': '0',
                    'fromtime': '2024-01-01', # '2025-01-01',
                    'totime': '2024-01-01', # f'{today}',
                    'catid': self.column[0],
                    'sfromtime': '',
                    'stotime': '',
                    'sub.x': '80',
                ...
                response = requests.get(
                    'http://down.foodmate.net/standard/search.php',
                    params=params,
                    proxies=get_proxies(),
                    headers=headers,
                    verify=False,
                ...
                div_list = response_html.xpath('//div[@class="bgt"]')
                logger.info(f'column:{self.column_domain} --- {self.page} --- 列表页数量：{len(div_list)}')
                if not div_list:
                    logger.error(f'列表页没数据 --- {self.column[1]} --- {self.page}')
                    return
                for div in div_list:
                    self.title = div.xpath('./span[@class="bzmc"]/a/@title')[0]
                    self.detail_url = div.xpath('./span[@class="bzmc"]/a/@href')[0]
                    F_有效性状态_en = div.xpath('./span[@class="bzmc"]/a/img/@src')[0].split('/')[-1].split('.')[0]
                    self.F_有效性状态 = F_有效性状态_mapping[F_有效性状态_en]
                    # self.publish_time = re.findall('(\d{4}-\d{2}-\d{2})', div.xpath('./ul[@class="ls_bq"]/span/text()')[0])[0]
                    logger.info(f"{self.F_有效性状态}, {self.detail_url}, {self.title}")
